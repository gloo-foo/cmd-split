...
func TestSplit_NonOptionArgumentIsClassifiedNotFolded(t *testing.T) {
	// A positional argument (a File-convertible string) is not one of split's
	// options: it must pass through the option fold untouched, leaving the
	// delimiter at its whitespace default, and the command still reads the
	// piped stream.
	lines, err := testable.TestLines(command.Split("ignored.txt"), "one two\n")
	assertion.NoError(t, err)
	assertion.Lines(t, lines, []string{"one", "two"})
}
